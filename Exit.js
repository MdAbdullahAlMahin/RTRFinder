...
export default function Exit({navigation}) {
  ...
  const nextpage = () => {
    navigation.navigate('NearExit',{Exit})
  }

  return (
      <SafeAreaView style={styles.container}>
          <StatusBar style="auto" />
          <Text style={padding(0,0,10,0)}>{navigation.getParam('location')} Exits:</Text>
          <Text style={padding(0,0,10,0)}>@mahin: enter exit choices here from storage</Text>
          ...
          />
             <TouchableOpacity 
              style ={styles.button}
              onPress={() => { onPressHandle(); nextpage();}}
              activeOpacity={0.5}
          >
              <Text>Submit</Text>
          </TouchableOpacity>
